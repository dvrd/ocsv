...
test_simd_find_delimiter :: proc(t: ^testing.T) {
    ...
    // Find first comma
    pos := cisv.find_delimiter_simd(data, ',', 0)
    testing.expect_value(t, pos, 5)

    // Find second comma
    pos = cisv.find_delimiter_simd(data, ',', 6)
    testing.expect_value(t, pos, 11)

    // Find third comma
    pos = cisv.find_delimiter_simd(data, ',', 12)
    testing.expect_value(t, pos, 15)

    // Not found
    pos = cisv.find_delimiter_simd(data, ',', 16)
...
test_simd_find_quote :: proc(t: ^testing.T) {
    ...
    pos := cisv.find_quote_simd(data, '"', 0)
    testing.expect_value(t, pos, 6)

    pos = cisv.find_quote_simd(data, '"', 7)
...
test_simd_find_newline :: proc(t: ^testing.T) {
    ...
    pos := cisv.find_newline_simd(data, 0)
    testing.expect_value(t, pos, 5)

    pos = cisv.find_newline_simd(data, 6)
    testing.expect_value(t, pos, 11)
}
...
test_simd_find_any_special :: proc(t: ^testing.T) {
    ...
    pos, found := cisv.find_any_special_simd(data, ',', '"', 0)
    testing.expect_value(t, pos, 5)
    testing.expect_value(t, found, byte(','))

    // Find newline at position 11
    pos, found = cisv.find_any_special_simd(data, ',', '"', 6)
    testing.expect_value(t, pos, 11)
    testing.expect_value(t, found, byte('\n'))

    // Find quote at position 15
    pos, found = cisv.find_any_special_simd(data, ',', '"', 12)
    testing.expect_value(t, pos, 15)
    testing.expect_value(t, found, byte('"'))
}
...
test_simd_parser_simple :: proc(t: ^testing.T) {
    ...
    ok := cisv.parse_csv_simd(parser, csv_data)
...
test_simd_vs_standard_performance :: proc(t: ^testing.T) {
    ...
    start := time.now()
    ok := cisv.parse_csv(parser1, csv_data)
    ...
    start = time.now()
    ok = cisv.parse_csv_simd(parser2, csv_data)
...
test_simd_availability :: proc(t: ^testing.T) {
    available := cisv.is_simd_available()
    arch := cisv.get_simd_arch()
...
test_parse_csv_auto :: proc(t: ^testing.T) {
    ...
    small_data := "a,b,c\n1,2,3\n"
    ok := cisv.parse_csv_auto(parser, small_data)
    ...
    ok = cisv.parse_csv_auto(parser, large_data)
