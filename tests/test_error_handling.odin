...
test_error_unterminated_quote :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, result.error.code, cisv.Parse_Error.Unterminated_Quote)
...
test_error_unterminated_quote_relaxed :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, result.warnings[0].code, cisv.Parse_Error.Unterminated_Quote)
}
...
test_error_invalid_character_after_quote :: proc(t: ^testing.T) {
    ...
    testing.expect(t, !result.success, "Parse should fail")
    testing.expect_value(t, result.error.code, cisv.Parse_Error.Invalid_Character_After_Quote)
...
test_error_empty_input :: proc(t: ^testing.T) {
    ...
    result := cisv.parse_csv_with_errors(parser, "")
    defer cisv.parse_result_destroy(&result)

    testing.expect(t, !result.success, "Parse should fail on empty input")
    testing.expect_value(t, result.error.code, cisv.Parse_Error.Empty_Input)
}
...
test_error_column_consistency :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, err.code, cisv.Parse_Error.Inconsistent_Column_Count)
...
Bob,25,SF`
    ...
    rows, result := cisv.parse_csv_safe(csv_data, config)
    defer {
        for row in rows {
            for field in row {
                delete(field)
...
test_parse_csv_safe_with_error :: proc(t: ^testing.T) {
    ...
    config := cisv.default_config()
    rows, result := cisv.parse_csv_safe(csv_data, config, .Fail_Fast)
    defer {
        for row in rows {
            for field in row {
                delete(field)
    ...
    testing.expect(t, !result.success, "Parse should fail")
    testing.expect(t, cisv.is_error(result.error), "Should have error")
...
test_parse_csv_safe_relaxed :: proc(t: ^testing.T) {
    ...
    rows, result := cisv.parse_csv_safe(csv_data, config, .Best_Effort)
    defer {
        for row in rows {
            for field in row {
                delete(field)
...
test_error_formatting :: proc(t: ^testing.T) {
    ...
    formatted := cisv.format_error(err)
...
test_error_context_extraction :: proc(t: ^testing.T) {
    ...
    ctx_str := cisv.get_context_around_position(data, error_pos, 10)
