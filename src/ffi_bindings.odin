...
// cisv_parser_create creates a new parser instance
...
// cisv_parser_destroy destroys a parser instance and frees all memory
// Parameters:
...
// cisv_parse_string parses a CSV string
// Parameters:
...
// cisv_get_row_count returns the number of rows parsed
// Parameters:
//   parser: pointer to Parser
...
// cisv_get_row gets a specific row by index
// Parameters:
//   parser: pointer to Parser
...
@(export, link_name="cisv_get_row")
cisv_get_row :: proc "c" (parser: ^Parser, row_index: c.int) -> [^]cstring {
    context = runtime.default_context()
...
// cisv_get_field_count gets the number of fields in a specific row
// Parameters:
//   parser: pointer to Parser
//   row_index: row index (0-based)
// Returns: number of fields in the row, or -1 if invalid index
@(export, link_name="cisv_get_field_count")
cisv_get_field_count :: proc "c" (parser: ^Parser, row_index: c.int) -> c.int {
    context = runtime.default_context()
